...
- Se reemplazo el hook de OPS(OnPlayerSpawn) por public normal, debido a problemas de lectura con Nex-AC al obtener la vida del jugador. (root)
- El sistema de administracion tuvo ligeros cambios, debido a complicaciones con la asignacion de permisos(flags) en pawncmd. 

### Changes 1.0 / 30/23-06 - 2024
- Se corrigio un error con el comando de Asignar permisos de Administracion. (thanks to Gatini)
- Se reemplazo y modifico un pequeño detalle en variables y funciones para la verificacion de los rangos/niveles administrativos. (Thanks Gatini)

### Changes 1.0 / 28/23-06 - 2024
- Se reemplazo el metodo de conexion a la base de datos, asi mismo se soluciono un problema de conexion. (ini a interno)
- Se realizaron cambios minimos en algunas funciones.
- La mayoria de los ALS fueron reemplazados por hooks, a excepcion de OnPlayerCommandPerformed. (no tocar este)

### Changes 1.0 / 22/23-06 - 2024

- Fue solucionado un inconveniente con la disminucion de vida al morir, la cual no funcionaba.
- Se soluciono un problema con la animacion de muerte, la cual no reproducia esta al morir y solo era visible para otro.
- Fue reparado un pequeño bug el cual mantenia a tod@s AFK tras un lapso de tiempo.
- Varios comandos fueron reparados y asi mismo modificados en relacion a sus variables.
- Fue reparado un pequeño bug en relacion a la DB, el cual no registraba correctamente los datos al darse admin IC.

----

### Changes 1.0 / 26-10 - 2023
 ...
 **Content:**
    ...
    |———— content\cmds      - Commands player, faction, admins
